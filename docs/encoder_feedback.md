...
2. **作動遅延モデル**: 受信した body 速度指令に **一次遅れ τ ＋ トランスポートむだ時間** を per-axis で付与してから物理に渡す（OC に現実的なプラントを与える）。→ 現在はこれを含む実機同定モデル（ゲイン・軸別の牽引限界・車輪周速の予算まで）を ID ごとに持つ。`[Actuation]` は `[RobotModel]` に置き換わった。**`docs/robot_motion_model.md` を参照。**
...
- `src/models/robot.cpp::advanceActuation` … 指令→適用速度の実機同定モデル（むだ時間・一次遅れ・定常ゲイン・軸別の牽引限界・車輪周速の予算）。`Observer::updateObjects` から毎物理フレーム。パラメータは `src/models/robotModel.h`。
...
既定（`[Encoder]` が全0）は理想エンコーダ。台の応答は `[RobotModel]`（`docs/robot_motion_model.md`）で決まり、`Enabled=false` にすると遅延なしの従来挙動に戻る。`blueRobotCount` は RAVEN が駆動するIDが spawn する程度に確保すること（11 推奨）。
